...
def FD_jpeg_encode(input_matrix):
	input_matrix = input_matrix.permute(0,2,3,1) # 将一个pytorch tensor数组转换为numpy数组
	...
	# 将numpy数组转换为pytorch tensor数组
	output3 = torch.from_numpy(output3)
	return output3

class FD_jpeg(object):		
	def __init__(self):
		pass

	def __call__(self, input_matrix):
		return FD_jpeg_encode(input_matrix)
